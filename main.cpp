...
private:
    ...
    void generateCircleVertices(Circle circle, int vertices) {
        ...
        for (int i = 0; i < vertices; i++) {
            float theta = i * angleBetweenPoints;
            ...
            globalVertexIndex++;
            if(i != vertices - 1) globalMeshData.indices.push_back(globalVertexIndex);
            else globalMeshData.indices.push_back(firstVertexIndex);
        }
    }
    ...
    void createInstance() {
        ...
        appInfo.pApplicationName = "Apollonian Gasket Application";
    ...
    void createVertexBuffer() {
        VkDeviceSize bufferSize = sizeof(globalMeshData.vertices[0]) * globalMeshData.vertices.size();
        ...
        memcpy(data, globalMeshData.vertices.data(), (size_t) bufferSize);
    ...
    void createIndexBuffer() {
        VkDeviceSize bufferSize = sizeof(globalMeshData.indices[0]) * globalMeshData.indices.size();
        ...
        void* data;
        vkMapMemory(device, stagingBufferMemory, 0, bufferSize, 0, &data);
        memcpy(data, globalMeshData.indices.data(), (size_t) bufferSize);
        vkUnmapMemory(device, stagingBufferMemory);
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
        ...
        vkCmdDrawIndexed(commandBuffer, static_cast<uint32_t>(globalMeshData.indices.size()), 1, 0, 0, 0);
